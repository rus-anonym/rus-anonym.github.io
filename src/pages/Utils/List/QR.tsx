...
const formats = [
  ...
  {
    title: "EAN-8 1D",
    url: "https://en.wikipedia.org/wiki/EAN-8",
  },
  {
    title: "EAN-13 1D",
...
const QRInfo = ({ result }: { result: Result }): JSX.Element => {
  ...
  return (
    <>
      ...
      <MiniInfoCell before={<Icon20ArticleOutline />} textWrap="full">
        Barcode Format:{" "}
        <Link target="_blank" href={formats[result.getBarcodeFormat()].url}>
...
const QR = (): JSX.Element => {
  ...
  if (isFileSelector && result === null) {
    return (
      <ImageLoader setResult={setResult} setFileSelector={setFileSelector} />
    );
  }
    ...
    return (
      <Group>
        <Placeholder
          ...
          action={
            <Button size="m" onClick={(): void => void requestCamera()}>
              {t("requestCamera")}
            </Button>
          }
        >
