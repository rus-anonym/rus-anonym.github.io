...
class Feminizator {
  ...
  public static findEndingData(wordEnding: string): IEndingData[] | undefined {
  ...
  public static makeFeminitive(
    ...
    for (const wordEnding of [word.slice(-4), word.slice(-3), word.slice(-2)]) {
      ...
      return {
        type: "feminitive",
        stem: endings[0].stem,
        endings: ([] as string[]).concat(...endings.map((x) => x.endings)),
  ...
  return (
    <Group
      header={
        <Header
          subtitle={
            feminizedWord.type === "exception" && feminizedWord.description
          }
        >
    ...
    >
      ...
      <Div>
        <Button
          ...
          appearance="neutral"
          onClick={(): void => {
            const url = router.url + "?word=" + base64url.encode(word);
