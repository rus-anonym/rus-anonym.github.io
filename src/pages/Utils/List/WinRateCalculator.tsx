...
const Result = ({
  ...
  if (result.count === Infinity) {
    return <Text>Достигнуть процента побед {desiredWinrate}% невозможно</Text>;
  }
      ...
      <form>
        <FormItem top="Общее количество матчей">
          <Input
            ...
            onChange={(e): void => setTotalMatchesCount(Number(e.target.value))}
            type="number"
          />
        </FormItem>
        <FormItem top="Текущий процент побед" status={totalWinrateStatus}>
          <Input
            min={0}
            max={100}
        ...
        <FormItem top="Желаемый процент побед" status={desiredWinrateStatus}>
          <Input
            min={0}
            max={100}
